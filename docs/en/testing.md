...
CI jobs `release-gates` and `test` must be green for a release. Ship the engine and `custom_components/klar_nlu` in the same cut: the integration only calls `POST /api/v2/parse`.
