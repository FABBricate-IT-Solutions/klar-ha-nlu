...
de/en are hand-written in src/lang/de_pack.rs and en_pack.rs and are never
overwritten here. When you add a LanguagePack field, update those two packs,
this template, and scripts/check_lang_packs.py (shared field checklist).
"""
...
def write_registry(langs: list[dict]) -> None:
    ...
    text = f"""{RUST_BANNER}//! Compiled language table. de/en are hand-written reference rows; generated packs follow.
...
"""
    ...
    mods = "\n".join(
        f"pub mod {item['mod']};" for item in sorted((row for row in langs if row["code"] not in ("de", "en")), key=lambda row: row["mod"])
    )
