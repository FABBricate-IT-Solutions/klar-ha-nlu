    ...
    lex.update(
        {
            ...
            "lock_v": first("lock_v", w.get("lock_v") or w.get("lock") or ["verrouille"]),
            ...
            "music": next(
                (word for word in (w.get("media") or []) if any(key in word.lower() for key in ("mus", "lagu", "cancion", "canto"))),
                (w.get("media") or ["music"])[-1],
            ),
            "too": first("too", ["too"]),
            "percent": first("percent", ["percent"]),
            "dim": first("dim", ["dimmer"]),
            "medium": first("medium", ["medium"]),
            "warm": first("warmwhite", ["warmwhite"]),
            "play": first("play", ["play"]),
            "pause": first("pause", ["pause"]),
            "next": first("next", ["next"]),
            "quiet": first("quiet", ["quieter"]),
            "hours": first("hours", ["hour"]),
            "calendar": first("calendar", ["calendar"]),
            "clock": first("clock", w.get("query") or ["time"]),
            "weather": first("weather", ["weather"]),
            "lamp": lamp,
