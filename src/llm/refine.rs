...
use super::refine_prompt::{refine_input, refine_prompt};
...
pub const REFINE_MAX_TOKENS: u32 = 128;
...
pub async fn refine(endpoint: &LlmEndpoint, request: RefineRequest) -> Result<RefineOutcome, LlmError> {
    ...
    let prompt = refine_prompt(&body.language, &body.personality, Some(body.extra_prompt.as_str()));
    let user = refine_input(&body.speech);
    let chat_req = ChatRequest {
        messages: vec![ChatMessage { role: "system".into(), content: prompt }, ChatMessage { role: "user".into(), content: user }],
        stream: Some(false),
