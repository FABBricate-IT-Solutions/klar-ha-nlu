...
use crate::home::policy::{fallback_climate, fallback_cover_area, is_infra};
...
fn guess_action(tokens: &[String], session: &Session, number: Option<i32>) -> Action {
    if number.is_some() {
        crate::parse::numbers::guess_numbered_action(
            tokens,
    ...
    } else if tokens.iter().any(|token| matches!(token.as_str(), "auch" | "too" | "also" | "well")) {
...
fn resolve_targets(
    ...
    if settings.mode == Mode::ContextOnly {
        return crate::parse::resolve::Resolved { areas: resolve(tokens, home, None).areas, entities: Vec::new(), ambiguous: Vec::new() };
    }
